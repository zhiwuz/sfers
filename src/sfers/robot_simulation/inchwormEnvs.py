...
class inchworm:
    def __init__(self):
        ...
        self.beta = 1.46e-4  #Condition 9(j) - beta_1
    ...
    def simReset(self):
        self.actuator2DDensity=8*self.subThickness+5.44*self.actThickness;
        self.actuator1DDensity=self.actuator2DDensity*self.width;
        self.actuatorMass=self.actuator1DDensity*self.actuatorLength;
        self.lateralFrictionCoefficients=[self.frictionLow for _ in range((self.N-1)*self.m+int(math.ceil(2*self.m/3)))]
        ...
        p.changeDynamics(self.planeId, -1, spinningFriction=0.0, rollingFriction=0.0, lateralFriction=self.groundFriction,
                         linearDamping=0.0, angularDamping=0.0,jointDamping=0.0)
        p.changeDynamics(self.planeId, 0, spinningFriction=0.0, rollingFriction=0.0, lateralFriction=self.groundFriction,
                         linearDamping=0.0, angularDamping=0.0,jointDamping=0.0)
    ...
    def TorVolThe(self,Theta,angularVelocity,Voltage):
        ...
        #halfEI=3.973*1e5;
        #thetaTarget=-1.6e-4*Voltage;
        thetaTarget=-self.beta*Voltage; 
        #thetaTarget=-1.4587e-3*Voltage;
    ...
    def simStep(self, actuatorVoltages):
        ...
        [self.theta,self.angularVelocities,self.positions,self.motorVoltages,self.Tor]=voltageTorqueControlStep(self.boxId,
                                                                                       actuatorVoltages,
                                                                                       self.TorVolThe, 
                                                                                       self.N, 
                                                                                       self.m, 
